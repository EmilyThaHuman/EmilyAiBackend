...
const handleFileUploadFunction = asyncHandler(async (req, res) => {
  ...
  const { workspaceId, folderId, userId, file } = req.body;
...
// Storage routes
const getStoredFilesByType = (req, res) =>
  handleFileRetrieval({ contentType: new RegExp(req.params.type, 'i') }, res);
const getStoredFilesBySpace = (req, res) =>
  handleFileRetrieval({ 'metadata.space': req.params.space }, res);
const getStoredFilesByPath = (req, res) =>
  handleFileRetrieval({ 'metadata.path': req.params.path }, res);
...
const handleFileOperation = (operation) => (req, res) =>
  handleDatabaseOperation(operation(req), res);
...
const getAssistantFileById = handleFileOperation(
  (req) => () => AssistantFile.findById(req.params.id)
);
