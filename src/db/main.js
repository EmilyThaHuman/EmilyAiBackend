...
//   file: (req, file) => {
//     logger.info(`File received: ${JSON.stringify(file)}`);
...
//   logger.info(`GridFsStorage connected to MongoDB: ${JSON.stringify(db[0])}`);
// });
...
//   logger.error(` Stream error: ${error.message} Configuration: ${JSON.stringify(conf)}`);
// });
...
const handleFileUpload = async (req, res) => {
  upload.single('file')(req, res, async (err) => {
    ...
    try {
      const { workspaceId, userId, folderId } = req.body;
...
//     // logger.info(`File upload successful: ${JSON.stringify(req.file)}`);
