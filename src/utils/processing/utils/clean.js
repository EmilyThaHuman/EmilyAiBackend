...
/**
 * Normalizes line breaks in the given code by replacing three or more consecutive line breaks with two line breaks.
 * @param {string} code - The input code string to normalize.
 * @returns {string} The normalized code with excessive line breaks reduced.
 */
...
function removeIndentation(code) {
  ...
  /**
   * Calculates the minimum indentation level among non-empty lines in an array of strings.
   * @param {string[]} lines - An array of strings representing lines of text.
   * @returns {number} The minimum number of leading whitespace characters found in non-empty lines.
   */
...
function unescapeHtml(code) {
   ...
   * @returns {string} The input string with HTML special characters replaced by their entity references
   */
...
function cleanCodeSnippetGeneral(snippet) {
  ...
  code = code
     ...
     * @param {Array<string>} lines - An array of strings to be filtered.
